    ...
    mlir::MemRefType memrefTy, bool ubIfReadOnly)
{
  ...
  else
  {
    ...
    // Wrote successfully
    st.wellDefined(op, Expr::mkForall(idxs, tInBounds.implies(mInfoAfter.checkWrite(!ubIfReadOnly))),
                   "write successful");
    ...
    const State &st, const llvm::SmallVector<mlir::OpFoldResult> &mixedOps)
{
  ...
  for (auto s : mixedOps)
  {
    vec.push_back(s.is<mlir::Value>() ? st.regs.get<ValTy>(s.get<mlir::Value>()) : Index(s.get<mlir::Attribute>().dyn_cast<mlir::IntegerAttr>().getInt()));
  }
  return vec;
}
    ...
    mlir::AffineExpr ae, const vector<T> &dimvars, const vector<T> &symbolvars)
{
  ...
  case mlir::AffineExprKind::Mul:
  {
    ...
    if (!lhs || !rhs)
      return {};
    return (ae.getKind() == mlir::AffineExprKind::Add) ? *lhs + *rhs : *lhs * *rhs;
  }
...
void encodeOp(State &st, mlir::arith::CmpIOp op, bool)
{
  ...
  auto fn = [&](Expr &&a0, Expr &&b0) -> Expr
  {
    ...
    case mlir::arith::CmpIPredicate::eq:
      return (a == b).toOneBitBV();
    case mlir::arith::CmpIPredicate::ne:
      return (a != b).toOneBitBV();
    case mlir::arith::CmpIPredicate::ule:
      return (a.ule(b)).toOneBitBV();
    case mlir::arith::CmpIPredicate::ult:
      return (a.ult(b)).toOneBitBV();
    case mlir::arith::CmpIPredicate::uge:
      return (a.uge(b)).toOneBitBV();
    case mlir::arith::CmpIPredicate::ugt:
      return (a.ugt(b)).toOneBitBV();
    case mlir::arith::CmpIPredicate::sle:
      return (a.sle(b)).toOneBitBV();
    case mlir::arith::CmpIPredicate::slt:
      return (a.slt(b)).toOneBitBV();
    case mlir::arith::CmpIPredicate::sge:
      return (a.sge(b)).toOneBitBV();
    case mlir::arith::CmpIPredicate::sgt:
      return (a.sgt(b)).toOneBitBV();
    }
...
void encodeOp(State &st, mlir::math::AbsFOp op, bool)
{
  ...
  encodeUnaryOp(st, op, arg0, [](Float &&a)
                { return a.abs(); }, {});
}
...
void encodeOp(State &st, mlir::math::AbsIOp op, bool)
{
  ...
  encodeUnaryOp(st, op, arg0, {}, [](Integer &&a)
                { return ((Expr)a).abs(); });
}
...
void encodeOp(State &st, mlir::math::ExpOp op, bool)
{
  ...
  encodeUnaryOp(st, op, arg0, [](auto &&a)
                { return Float::exp(a); }, {});
}
  ...
  auto unaryFn = [elemTy, &op](smt::Expr &&elem0) -> smt::Expr
  {
    ...
    else
    {
      ...
      Expr e1 = Float(Expr::mkIte(
                          (Expr)elem.cmp(olt, minval) == one, minval, elem),
                      elemTy);
      Expr e2 = Float(Expr::mkIte(
                          (Expr)maxval.cmp(olt, elem) == one, maxval, e1),
                      elemTy);
                                ...
                                mlir::ArrayRef<int64_t> padding)
{
                    ...
                    { return a == 0; }))
  {
    ...
    auto zero = arg_use_neg_zero.getValue() ? *getIdentity(elemTy) : *getZero(elemTy);
...
void encodeOp(State &st, mlir::tosa::GatherOp op, bool)
{
  ...
  st.wellDefined(op, Expr::mkForall(indVars, inBounds.implies(std::move(idxInBounds) & std::move(inputInBounds))),
                 "indices and input's indices are inbounds");
  st.wellDefined(op, Expr::mkForall(indVars, inBounds.implies(std::move(isInitialized))),
        ...
        outputValMap = nullopt)
{
  ...
  encodeBlock(newst, block, /*print ops*/ false, /*encode mem writes*/ false, [&yieldedValues](mlir::Operation *op, int opindex)
              {
        ...
        return false; }, [&welldef, &newst](mlir::Operation *op)
...
template <>
void encodeOp(State &st, mlir::linalg::DepthwiseConv2DNhwcHwcmOp op,
...
template <>
void encodeOp(State &st, mlir::linalg::Conv2DNchwFchwOp op, bool encodeMemWriteOp)
{
  if (!op.hasPureTensorSemantics() && !encodeMemWriteOp)
    throw UnsupportedException(op.getOperation());
...
template <>
void encodeOp(State &st, mlir::linalg::Conv2DNhwcHwcfOp op, bool encodeMemWriteOp)
{
  if (!op.hasPureTensorSemantics() && !encodeMemWriteOp)
    throw UnsupportedException(op.getOperation());
...
static void encodeLinalgPooling(State &st, T op)
{
  ...
  {
    mlir::Type elemTy = op.getOutputs()[0].getType().template cast<mlir::MemRefType>().getElementType();
    if (!elemTy.isa<mlir::FloatType>())
      throw UnsupportedException(op.getOperation(), "Unsupported type");
...
template <>
void encodeOp(State &st, mlir::tensor::EmptyOp op, bool)
{
...
void encodeOp(State &st, mlir::tensor::InsertSliceOp op, bool)
{
  ...
  if (rank != op.getOperand(1).getType().dyn_cast<mlir::ShapedType>().getRank() || rank != res.getType().dyn_cast<mlir::ShapedType>().getRank())
    throw UnsupportedException(op.getOperation(),
  ...
  // initialized as well.
  st.wellDefined(op,
                 Expr::mkForall(indVars, (tgtwb & !cond).implies(tgt.isInitialized(indVars))), "tgt initialized");
...
void encodeOp(State &st, mlir::tosa::AddOp op, bool)
{
  ...
  encodeBinaryOp(st, op, arg0, arg1, [](auto &&a, auto &&b)
                 { return a.add(b); }, [](auto &&a, auto &&b)
                 { return (Expr)a + (Expr)b; });
}
...
void encodeOp(State &st, mlir::tosa::SubOp op, bool)
{
  ...
  encodeBinaryOp(st, op, arg0, arg1, [](auto &&a, auto &&b)
                 { return a.add(b.neg()); }, [](auto &&a, auto &&b)
                 { return (Expr)a - (Expr)b; });
}
...
void encodeOp(State &st, mlir::tosa::NegateOp op, bool)
{
  ...
  encodeUnaryOp(st, op, arg0, [](auto &&a)
                { return a.neg(); }, [](auto &&a)
                { return Expr::mkBV(0, a.bitwidth()) - (Expr)a; });
}
...
#define ADD(vec, ee)                                                                                            \
  {                                                                                                             \
    vec.push_back(op.isDynamic##ee(i) ? st.regs.get<Index>(op.getDynamic##ee(i)) : Index(op.getStatic##ee(i))); \
  }
    ADD(offsets, Offset);
    ADD(sizes, Size);
    ADD(strides, Stride);
...
void encodeOp(State &st, mlir::linalg::DotOp op, bool encodeMemWrite)
{
  ...
  if (outputTy.getElementType() !=
      inputOps[0].getType().dyn_cast<mlir::TensorType>().getElementType())
    ...
    map<string, Expr> &welldefs)
{
  ...
  unsigned idx;
  if (p1.match(&ops.back()) || p3.match(&ops.back()))
    idx = 1;
  else if (p2.match(&ops.back()) || p4.match(&ops.back()))
    idx = 0;
  else
  ...
  encodeBlock(newst, block, /*print ops*/ false, /*encode mem writes*/ false, [instcount, &lastarg, &the_op](mlir::Operation *op, int opindex)
              {
        ...
        return false; }, [&opsWelldef, &newst](mlir::Operation *op)
...
void encodeOp(State &st, mlir::linalg::GenericOp op, bool encodeMemWriteOp)
{
  ...
  if (!std::all_of(block.args_begin(), block.args_end(),
                   [](auto &arg)
                   { return arg.getType().isSignlessIntOrFloat(); }))
    throw UnsupportedException(op.getOperation(),
  ...
  else if (op.hasPureBufferSemantics())
  {
    ...
    for (auto opi : op.getOutputs())
    {
      ...
      {
        if (!opj.getType().isa<mlir::MemRefType>())
          continue;
    ...
    function<void(mlir::Operation *)> callbackAfterEnc)
{
  ...
  for (auto &op : block)
  {
    ...
    if (checkBeforeEnc && checkBeforeEnc(&op, index))
      continue;
